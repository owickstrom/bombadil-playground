...
- `spellcheck.ts`, `wordExtractor.ts`, and `custom.dic` live under
...
*not* add typos to `custom.dic`.

**False positive — legitimate word the dictionary doesn't know.** Two
options, choose by context:

- *Add to `custom.dic`* when the word is reusable across posts (proper
